...
app.post("/api/rooms/:roomId/join", async (req, res) => {
  ...
  if (!room || !characterId || !db.characters[characterId]) {
  ...
  if (!alreadyInRoom) {
    ...
    pushLog(room, `${db.characters[characterId].name} 加入了房间。`);
    await persist();
  }
...
app.post("/api/rooms/:roomId/action", async (req, res) => {
  ...
  if (!actingCharacter || !choice || !scene) {
...
async function tryAiNarration(context) {
  ...
  const prompt = [
    ...
    "请返回 JSON，格式为 {\"title\":\"\",\"body\":\"\",\"logText\":\"\"}。"
  ].join("\n");
